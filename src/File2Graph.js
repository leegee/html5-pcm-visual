...
*/
	...
	var exports = function File2Graph( options ) {
		...
		if ( !this.options.width ) {
			this.options.width = parseInt( getStyle( this.element, 'width' ) );
		}
		if ( !this.options.height ) {
			this.options.height = parseInt( getStyle( this.element, 'height' ) );
		}

		if ( typeof this.options.playable === "string" &&
			!this.options.playable.match( /^(0|false|)$/ )
		) {
			this.options.playable = true;
	...
	exports.prototype.fireEvent = function ( eventName ) {
		var methodName = 'on' + eventName.substr( 0, 1 )
			.toUpperCase() + eventName.substr(
				1 ),
			self = this,
	...
	exports.prototype.initGraphics = function () {
		...
		gfxEl.forEach( function ( el ) {
			this[ el ] = document.createElement( el );
			this[ el ].setAttribute( 'class', this.element.getAttribute( 'class' ) );
			this[ el ].style.backgroundColor = this.options.background;
            [ 'width', 'height' ].forEach( function ( i ) {
				this[ i ] = parseInt( this.options[ i ] );
				var unit = ( this[ i ] === this.options[ i ] ) ? 'px' : '';
    ...
    */
	exports.prototype.render = function () {
		...
		if ( this.options.graphwaveform ) {
			...
			var xFactor = this.canvas.width / cd[ 0 ].length;
			...
			for ( var i = 0; i < cd[ 0 ].length; i += parseInt( this.options.step, 10 ) ) {
				...
				var x = i * xFactor,
					y = ( v / this.buffer.numberOfChannels );
				// console.assert( y < 1 );
				y = ( this.canvas.height * y / 2 ) + ( this.canvas.height / 2 );
    ...
    */
	exports.prototype.offline_overlayImg = function ( e ) {
		...
		if ( typeof this.overlay.pxPerSec === 'undefined' ) {
			this.overlay.pxPerSec = this.canvas.width / this.buffer.duration;
