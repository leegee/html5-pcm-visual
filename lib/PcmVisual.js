define( [ 'Sound', 'GUID' ], function ( Sound, GUID ) {
	...
	var exports = function PcmVisual( options ) {
		...
		this.options.element.appendChild(
			range.createContextualFragment(
				"<canvas class='app' id='" + this.id + "' width='" + this.options.width +
	...
	exports.prototype.renderFrame = function () {
		...
		for ( var gen = 0; gen < this.history.length; gen++ ) {
			this.ctx.beginPath();
			this.ctx.fillStyle = this.ctx.strokeStyle = 'rgba(' + this.options.rgbForeground +
